...
func (g *JavaGen) genProxyImpl(name string) {
	...
	g.Printf("      Seq1.incGoRef(refnum, this);\n")
...
func (g *JavaGen) genStruct(s structInfo) {
	...
	if jinf != nil {
		impls = append(impls, "Seq1.GoObject")
	...
	if jinf == nil || jinf.genNoargCon {
		...
		g.Printf("%s(int refnum) { this.refnum = refnum; Seq1.trackGoRef(refnum, this); }\n\n", n)
		if len(cons) == 0 {
			// Generate default no-arg constructor
			g.Printf("public %s() { this.refnum = __New(); Seq1.trackGoRef(refnum, this); }\n\n", n)
...
func (g *JavaGen) genConstructor(f *types.Func, n string, jcls bool) {
	...
	g.Printf(");\n")
	g.Printf("Seq1.trackGoRef(refnum, this);\n")
	g.Outdent()
	g.Printf("}\n\n")
...
func (g *JavaGen) GenJava() error {
	...
	g.Printf("static {\n")
	g.Indent()
	g.Printf("Seq1.touch(); // for loading the native library\n")
	if g.Pkg != nil {
	...
	for _, iface := range g.interfaces {
		...
		g.Printf(" implements Seq1.Proxy, %s {\n", g.javaTypeName(n))
		g.Indent()
		g.genProxyImpl("proxy" + n)
		g.Printf("proxy%s(int refnum) { this.refnum = refnum; Seq1.trackGoRef(refnum, this); }\n\n", n)
